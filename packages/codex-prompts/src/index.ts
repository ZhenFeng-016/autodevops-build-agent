...
export function buildCommandInferencePrompt(input: {
  ...
  return [
    ...
    'Infer Runtime Contract deployment fields separately. A generic package.json build script is not automatically a frontend build.',
    'Classify serverBuildCommand from server/library compilation evidence, frontendBuildCommand from browser-target build evidence, frontendDistDir from the actual bundler/config output path, and pm2StartCommand from a production server entrypoint.',
    'The selected deploy mode provides deployment context only; it is never sufficient evidence for a build or start command.',
    'Reject development/watch commands as PM2 production start commands. Prefer a production script or a source-verified server entrypoint and report a blocker when neither exists.',
    'For Oak repositories, inspect scripts such as build, build:lib, build:web and files such as scripts/startServer.js plus the installed Oak CLI/config when available. Oak build:web commonly targets the web subdirectory, but report web/build only when repository or Oak CLI evidence supports it.',
    ...
    jsonSchemaText({
      ...
      runtimeContract: {
        serverBuildCommand: 'exact server build command, or empty string when not applicable/unsupported',
        frontendBuildCommand: 'exact browser frontend build command, or empty string when no static frontend is built',
        frontendDistDir: 'repository-relative static frontend output directory, or empty string when no static frontend is built',
        pm2StartCommand: 'exact production PM2 application start command, or empty string outside PM2 / when evidence is insufficient',
        evidence: {
          serverBuildCommand: ['specific package.json script or file evidence'],
          frontendBuildCommand: ['specific package.json script or file evidence'],
          frontendDistDir: ['specific bundler/Oak CLI/config evidence'],
          pm2StartCommand: ['specific production script or server entrypoint evidence'],
        },
      },
      database: {
